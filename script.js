  ...
  try {
    ...
    try {
      ...
      if (e.code === 'auth/user-not-found') {
        const db = getDB();
        if (!db.users[username]) return; // לא מורשה
...
function doRegister() {
  ...
  const requireApproval = getSettings().requireRegistrationApproval !== false;
  const status = requireApproval ? 'pending' : 'active';
  const email = document.getElementById('regEmail')?.value.trim() || '';

  // Use SHA-256 (same as doLogin) — prevents mismatch on first login
...
function saveNewEmployee() {
  ...
  _sha256(pass).then(sha256hash => {
    ...
    showToast(`✅ עובד ${name} נוסף — יידרש לשנות סיסמה בכניסה הראשונה`, 'success');
    renderAdmin();
  });
}
...
function doResetPassword() {
  ...
  _sha256(newPass).then(hash => {
    ...
    db.users[passwordTargetUser].mustChangePassword = true; // force change on next login
    saveDB(db);
    closeModal('resetPasswordModal');
    showToast(`✅ סיסמת ${db.users[passwordTargetUser].fullName} אופסה — יידרש שינוי בכניסה הבאה`, 'success');
  });
}
...
async function changeAdminPassword() {
  ...
  if (!newP || newP.length < 4) {
    showToast('⚠️ הסיסמה חייבת להיות לפחות 4 תווים', 'warning'); return;
  }
  if (newP !== newP2) {
    showToast('⚠️ הסיסמאות אינן תואמות', 'warning'); return;
  }

  // Support both SHA-256 and legacy hash for current password verification
  const _legacyHash = (p) => { let h=0; for(let i=0;i<p.length;i++){h=((h<<5)-h)+p.charCodeAt(i);h=h&h;} return 'h'+Math.abs(h).toString(36)+p.length; };
  const sha256current = await _sha256(current);
  if (adminUser.password !== sha256current && adminUser.password !== _legacyHash(current)) {
...
function applyBulkImport() {
  ...
  // Pre-hash the temp password with SHA-256 so first login works without migration
  _sha256(tempPass).then(hashedPass => {
...
function openAIFromSelector() {
  ...
  // Only show welcome if chat is completely empty (first open)
...
function doChangePassword() {
  ...
  // Verify current password (supports both SHA-256 and legacy hash)
  _sha256(current).then(sha256current => {
    const legacyCurrent = user.password.startsWith('h') ? user.password : null;
    const hashPass_current = (function(p) {
      let h = 0;
      for (let i = 0; i < p.length; i++) { h = ((h << 5) - h) + p.charCodeAt(i); h = h & h; }
      return 'h' + Math.abs(h).toString(36) + p.length;
    })(current);
    if (user.password !== sha256current && user.password !== hashPass_current) {
...
function importBackup(input) {
  ...
  reader.onload = function(e) {
    try {
      ...
      // Preserve current admin password if not in backup
      const curDB = getDB();
      if (!data.users.admin && curDB.users.admin) {
        data.users.admin = curDB.users.admin;
      }
      _saveDBLocal(data);
