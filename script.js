...
function renderManagerDashboard() {
  ...
  // מחלקות של המנהל הנוכחי (רלוונטי רק למנהל מחלקה)
  const deptMgrs = db.deptManagers || {};
  const myManagedDepts = seeAll ? null : Object.keys(deptMgrs).filter(d => deptMgrs[d] === currentUser.username);
  // role=manager בלי שיוך מחלקה — רואה את כל המחלקות
  const myDeptSet = myManagedDepts && myManagedDepts.length > 0 ? new Set(myManagedDepts) : null;

  // פונקציית סינון עובד לפי מחלקה
  function inScope(user) {
    if (seeAll) return true;
    if (!myDeptSet) return true; // מנהל ללא שיוך — רואה הכל
    const uDept = Array.isArray(user.dept) ? user.dept[0] : user.dept;
    return myDeptSet.has(uDept);
  }

  const scopedUsers = Object.values(db.users).filter(u => u.status !== 'pending' && inScope(u));
  ...
  const totalEmployees = scopedUsers.filter(u => u.role === 'employee' || u.role === 'manager' || u.role === 'senior_manager').length;
  ...
  if(teamEl) {
    ...
    const rows = scopedUsers.filter(u=>u.role==='employee').map(u=>{
      const cb = calcBalance(u.username, year);
...
function renderEmployeeScores() {
  ...
  const db = getDB();
  const year = new Date().getFullYear();
  // סינון לפי scope של המנהל
  const _isAdminES = currentUser.role === 'admin' || currentUser.role === 'accountant';
  const _isSeniorES = currentUser.role === 'senior_manager' || currentUser.username === CEO_USERNAME;
  const _seeAllES = _isAdminES || _isSeniorES;
  const _deptMgrsES = db.deptManagers || {};
  const _myDeptsES = _seeAllES ? null : new Set(Object.keys(_deptMgrsES).filter(d => _deptMgrsES[d] === currentUser.username));
  const users = Object.values(db.users).filter(u => {
    if (!isUserActive(u) || u.role === 'admin' || u.role === 'accountant') return false;
    if (_seeAllES || !_myDeptsES || _myDeptsES.size === 0) return true;
    const uDept = Array.isArray(u.dept) ? u.dept[0] : u.dept;
    return _myDeptsES.has(uDept);
  });
