...
const DazuraAI = (() => {
  ...
  const INTENT_RULES = [
    ...
    // WHO + any date
    { name:'who_vacation',  score: t=>/מי (ב|הוא|היא|נמצא|יצא|בחופשה|חופש|נופש|נהנה|נעלם|נח|נופשים?|נהנים?|נחים?)|מי חופשה|מי יצא (לחופש|לנפוש|לנוח|ליהנות|מוקדם)|מי לא (מגיע|הגיע|כאן|נמצא|עובד|התייצב|נראה)|מי נעדר|מי חסר|מי לא פה|מי אין/.test(t)?10:0 },
    ...
    { name:'team_status',   score: t=>/מצב (הצוות|הצוות|הכיתה|הארגון|החברה)|הצוות (היום|מחר|השבוע)|עמיתי|חברי הצוות|מה קורה.*היום|מה המצב.*היום|מה נשמע.*עבודה|מי (כאן|פה|נמצא).*היום|מצב כללי/.test(t)?10:0 },
    // Admin
