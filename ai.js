...
// ============================================================
// BUILTIN Q&A — 111 שאלות מובנות (תמיד זמינות ללא טעינה)
// ============================================================
const BUILTIN_QA = [
  {question:"מה היתרה שלי?",aliases:["כמה ימי חופשה נשארו לי","כמה ימים יש לי","כמה חופשה נשאר לי","מה נשאר לי"],answer:"לחישוב יתרה עדכנית שאל את ה-AI: מה היתרה שלי - הוא יחשב לפי המכסה השנתית הצבירה החודשית והימים שניצלת.",tags:"יתרה,חישוב,חופשה"},
  {question:"כמה ימים צברתי החודש?",aliases:["צבירה חודשית","כמה יום בחודש","כמה אני צובר"],answer:"הצבירה החודשית = מכסה שנתית חלקי 12. לדוגמה: מכסה של 24 ימים שווה 2 ימים לחודש.",tags:"יתרה,צבירה,חישוב"},
  {question:"מתי כדאי לי לקחת חופשה?",aliases:["מה המלצה לחופשה","מתי הכי כדאי","תמליץ מתי לקחת"],answer:"כדאי לפזר חופשות לאורך השנה. אם היתרה גבוהה מעל 15 יום מומלץ לתכנן לפחות 2-3 ימים בחודש.",tags:"המלצה,תחזית,חופשה"},
  {question:"מה תחזית היתרה לסוף השנה?",aliases:["כמה יהיה לי בדצמבר","תחזית סוף שנה","כמה נשאר עד סוף השנה"],answer:"שאל: מה תחזית היתרה לסוף השנה - ה-AI יחשב לפי קצב הניצול הנוכחי וימי הצבירה שנותרו.",tags:"תחזית,יתרה,סוף שנה"},
  {question:"מה קורה לימים שלא ניצלתי?",aliases:["ימים שנשארו","מה עם ימים צבורים","ימים פגים"],answer:"מדיניות ניצול ימים צבורים נקבעת על ידי ההנהלה. פנה למנהל שלך או למשאבי אנוש לבירור מדיניות הפקיעה.",tags:"יתרה,מדיניות,ימים צבורים"},
  {question:"כמה ימי חופשה מגיעים לי בשנה?",aliases:["מה המכסה השנתית","כמה ימים זכאי","מה הזכאות"],answer:"המכסה השנתית נקבעת לפי ותק תפקיד וחוזה עבודה. לבדיקת המכסה האישית שאל: מה המכסה שלי?",tags:"מכסה,זכאות,שנתי"},
  {question:"האם ניצלתי חופשה החודש?",aliases:["כמה ימים לקחתי החודש","ניצול חודשי","מה לקחתי החודש"],answer:"שאל את ה-AI: כמה ימים ניצלתי השנה - הוא יפרט לפי חודשים.",tags:"ניצול,חודשי,מעקב"},
  {question:"תחשב לי כמה נשאר אם אקח 3 ימים",aliases:["כמה יישאר אם אקח","סימולציה ימים","אם אקח חופשה"],answer:"שאל: תחשב לי כמה נשאר אם אקח 3 ימים - ה-AI יפחית מיתרתך ויראה מה יישאר.",tags:"סימולציה,חישוב,יתרה"},
  {question:"מה היתרה שלי לשנה הבאה?",aliases:["ימים לשנה הבאה","יתרה 2026","צבירה לשנה הבאה"],answer:"יתרת השנה הבאה תחל מהיתרה הצבורה שלא נוצלה בהתאם למדיניות החברה ועוד הצבירה החודשית של השנה החדשה.",tags:"יתרה,שנה הבאה,תכנון"},
  {question:"כמה ימי חופשה ניצלתי השנה?",aliases:["סהכ חופשות השנה","ניצול שנתי","כמה ימים לקחתי"],answer:"שאל: כמה ימים ניצלתי השנה - ה-AI יציג פירוט מלא כולל ימים מלאים וחצאי ימים.",tags:"ניצול,שנתי,מעקב"},
  {question:"איך מגישים בקשת חופשה?",aliases:["איך לוקחים חופשה","איך להגיש חופשה","תהליך בקשת חופשה","איך מבקשים חופשה"],answer:"כניסה למערכת - לשונית לוח שנה - לחץ על תאריך הרצוי - בחר סוג יום מלא או חצי יום או WFH - אשר. הבקשה תועבר אוטומטית למנהל המחלקה לאישור.",tags:"הגשה,בקשה,תהליך"},
  {question:"איך מבטלים בקשת חופשה?",aliases:["לבטל חופשה","ביטול בקשה","איך מסירים חופשה"],answer:"עד לאישור המנהל: לחץ על היום בלוח השנה ובחר בטל בקשה. לאחר אישור: פנה ישירות למנהל המחלקה לביטול ידני.",tags:"ביטול,בקשה,תהליך"},
  {question:"מה ההבדל בין יום מלא לחצי יום?",aliases:["חצי יום חופשה","יום מלא חופשה","האם יש חצי יום"],answer:"יום מלא = 1 יום מהיתרה. חצי יום = 0.5 יום מהיתרה. ניתן לבחור חצי יום בעת הגשת הבקשה.",tags:"חצי יום,יום מלא,ניצול"},
  {question:"האם אפשר להגיש בקשה על תאריכים שעברו?",aliases:["חופשה רטרואקטיבית","תאריך עבר","אחורה בזמן"],answer:"ניתן לתאם מול המנהל ואדמין המערכת לעדכון ידני. הגשה עצמאית רטרואקטיבית תלויה בהגדרות המנהל.",tags:"רטרואקטיבי,עבר,עדכון"},
  {question:"כמה מראש צריך להגיש בקשת חופשה?",aliases:["כמה מוקדם","מראש כמה ימים","הודעה מוקדמת"],answer:"אין מגבלה טכנית במערכת אך מדיניות ההודעה המוקדמת נקבעת על ידי המנהל. מומלץ לתאם מראש לפחות שבוע קדימה.",tags:"הודעה מוקדמת,מדיניות,בקשה"},
  {question:"מה קורה אם שניים מהצוות מבקשים אותם ימים?",aliases:["חפיפת חופשות","שני עובדים אותו יום","חופשה מקבילה"],answer:"המנהל מקבל התראה על חפיפה ומחליט האם לאשר שתי הבקשות. בחר תאריכים בתיאום מראש עם עמיתים.",tags:"חפיפה,מקבילה,אישור"},
  {question:"האם אפשר להגיש כמה ימים ברצף?",aliases:["חופשה של שבוע","כמה ימים ברצף","חופשה ארוכה"],answer:"כן - לחץ על כל יום בנפרד בלוח השנה. לחופשות ארוכות פנה לאדמין לעדכון ידני של טווח תאריכים.",tags:"ברצף,ימים רצופים,בקשה"},
  {question:"מתי הבקשה מגיעה למנהל?",aliases:["מתי מנהל רואה","אישור מנהל","מתי נשלחת הבקשה"],answer:"מיד עם ההגשה - המנהל מקבל הודעה ורואה את הבקשה בלוח המנהל תחת בקשות ממתינות.",tags:"מנהל,אישור,תזמון"},
  {question:"איך יודעים אם הבקשה אושרה?",aliases:["סטטוס בקשה","אושרה חופשה","האם אישרו"],answer:"הצבע בלוח השנה ישתנה לירוק אם אושרה או אדום אם נדחתה. שאל את ה-AI: מה סטטוס הבקשה שלי?",tags:"סטטוס,אישור,מעקב"},
  {question:"מה קורה אם הבקשה נדחתה?",aliases:["בקשה נדחתה","חופשה לא אושרה","מה עושים אם דחו"],answer:"תקבל הודעה עם סיבת הדחייה אם צוינה. ניתן לפנות למנהל לבירור ולהגיש בקשה לתאריכים חלופיים.",tags:"דחייה,בקשה,תהליך"},
  {question:"מה ההבדל בין חופשה ל-WFH?",aliases:["עבודה מהבית","WFH","חופשה מול WFH"],answer:"חופשה = יום חופש מהיתרה. WFH = עבודה מהבית לא נוכח פיזית ולא נחשב כחופשה. WFH לא מקטין את יתרתך.",tags:"WFH,עבודה מהבית,סוגים"},
  {question:"כיצד מדווחים על יום מחלה?",aliases:["איך מדווחים מחלה","יום חולה","איך מדווח מחלה"],answer:"לשונית שעון נוכחות ואז דיווח מחלה - או פנה למנהל לעדכון ידני. יום מחלה אינו נוכה מיתרת החופשה.",tags:"מחלה,דיווח,נוכחות"},
  {question:"האם ימי מחלה נספרים ביתרת חופשה?",aliases:["מחלה מהיתרה","האם מחלה מקטינה יתרה","ספירת מחלה"],answer:"לא - ימי מחלה אינם מקוזזים מיתרת החופשה. הם נרשמים בנפרד.",tags:"מחלה,יתרה,חישוב"},
  {question:"מה זה ימי חג ואיך הם מחושבים?",aliases:["חגים","ימי חג","יום חג בחופשה"],answer:"ימי חג רשמיים אינם נחשבים כחופשה ואינם מקוזזים מהיתרה. אם חג חל במהלך חופשה מאושרת אותו יום אינו נספר.",tags:"חגים,חישוב,ימי חג"},
  {question:"האם ערב חג נחשב לחצי יום?",aliases:["ערב חג","לפני חג","יום לפני חג"],answer:"תלוי במדיניות החברה. בחברות רבות ערב חג מחייב נוכחות עד שעה מסוימת. בדוק עם ההנהלה.",tags:"ערב חג,מדיניות,חצי יום"},
  {question:"מה ההבדל בין חופשה לחלת?",aliases:["חלת","ללא תשלום","חופשה מיוחדת"],answer:"חופשה רגילה מנוכה מהיתרה עם תשלום שכר מלא. חלת היא הסדר נפרד שיש לתאם עם ההנהלה ומשאבי אנוש.",tags:"חלת,ללא תשלום,מדיניות"},
  {question:"איך רואים את לוח החופשות שלי?",aliases:["לוח שנה","היכן רואים חופשות","תצוגת חופשות"],answer:"לשונית לוח שנה מציגה את כל הימים שדווחת בהם: חופשה בכחול ו-WFH בסגול ומחלה באדום.",tags:"לוח שנה,תצוגה,ניווט"},
  {question:"מה אפשר לראות בתצוגה השנתית?",aliases:["תצוגה שנתית","תצוגת שנה","מפת חום שנתית"],answer:"לשונית שנתי מציגה מפת חום של כל ימי השנה - ניתן לראות בבת אחת אילו חודשים עמוסים ואיפה נשארת פנוי.",tags:"שנתי,מפת חום,תצוגה"},
  {question:"האם אפשר לראות את חופשות הצוות?",aliases:["חופשות עמיתים","מי בחופשה","לוח הצוות"],answer:"כן - שאל את ה-AI: מי בחופשה השבוע - תקבל רשימת נעדרים בחברה.",tags:"צוות,נעדרים,נוכחות"},
  {question:"איך מייצאים את לוח החופשות?",aliases:["ייצוא חופשות","להוריד נתונים","ייצוא לוח"],answer:"לשונית דוח אישי ואז לחצן ייצא לאקסל - יוריד קובץ CSV עם כל ימי החופשה שלך.",tags:"ייצוא,דוח,אקסל"},
  {question:"האם הלוח מסונכרן עם Google Calendar?",aliases:["גוגל קלנדר","סנכרון יומן","iCal"],answer:"כרגע אין אינטגרציה אוטומטית עם Google Calendar. ניתן לייצא CSV ולייבא ידנית.",tags:"גוגל קלנדר,סנכרון,אינטגרציה"},
  {question:"מי מאשר את הבקשה שלי?",aliases:["מי אחראי לאישור","למי הבקשה הולכת","מי המנהל שלי"],answer:"הבקשה נשלחת אוטומטית למנהל המחלקה שהוגדר עבור המחלקה שלך.",tags:"אישור,מנהל,תהליך"},
  {question:"תוך כמה זמן הבקשה מאושרת?",aliases:["זמן אישור","כמה זמן לאשר","SLA אישור"],answer:"אין מגבלה מחייבת במערכת. המנהל מקבל התראה מיידית. אם עברו 48 שעות ניתן לפנות למנהל ישירות.",tags:"זמן,אישור,SLA"},
  {question:"מה קורה אם המנהל לא מגיב?",aliases:["מנהל לא ענה","אין אישור","ממתין זמן רב"],answer:"לאחר 48 שעות ללא תגובה פנה למנהל ישירות. אדמין יכול לראות בקשות שממתינות מעל 48 שעות.",tags:"ממתין,מנהל,48 שעות"},
  {question:"האם אפשר לראות מי אישר את הבקשה?",aliases:["מי אישר","היסטוריית אישורים","לוג אישורים"],answer:"שאל את ה-AI: מה סטטוס הבקשה שלי - יוצגו פרטי הסטטוס האחרון.",tags:"אישור,היסטוריה,לוג"},
  {question:"איך מדווחים כניסה ויציאה?",aliases:["שעון נוכחות","דיווח שעות","כניסה יציאה"],answer:"לשונית שעון נוכחות - לחץ כניסה בהגעה ויציאה בסיום. ניתן גם לדווח WFH מאותה לשונית.",tags:"שעון,נוכחות,כניסה יציאה"},
  {question:"מה קורה אם שכחתי לדווח כניסה?",aliases:["שכחתי לדווח","תיקון שעות","שעות שגויות"],answer:"פנה למנהל לתיקון ידני. מנהל יכול לעדכן שעות נוכחות בלוח המנהל.",tags:"תיקון,שעות,נוכחות"},
  {question:"האם WFH נחשב כנוכחות?",aliases:["עבודה מהבית נוכחות","WFH נחשב","מהבית שווה נוכחות"],answer:"כן - WFH נחשב כיום עבודה מלא ומופיע בדיווח הנוכחות. אינו מנכה מיתרת החופשה.",tags:"WFH,נוכחות,חישוב"},
  {question:"כמה ימי WFH מותר לי?",aliases:["ימי WFH","מגבלת WFH","כמה פעמים בשבוע מהבית"],answer:"מדיניות WFH נקבעת על ידי ההנהלה ויכולה להשתנות לפי תפקיד ומחלקה. פנה למנהל לבירור.",tags:"WFH,מדיניות,מגבלה"},
  {question:"איך רואים כמה ימי WFH לקחתי?",aliases:["ספירת WFH","היסטוריית WFH","כמה פעמים מהבית"],answer:"שאל את ה-AI: כמה ימים ניצלתי השנה - הוא מפרט WFH בנפרד.",tags:"WFH,מעקב,היסטוריה"},
  {question:"איך מוציאים דוח אישי?",aliases:["דוח אישי","ייצוא דוח","להוריד דוח"],answer:"לשונית דוח אישי - בחר תקופה - לחץ ייצא. הדוח יוריד בפורמט CSV.",tags:"דוח,ייצוא,אישי"},
  {question:"מה כולל הדוח האישי?",aliases:["תוכן הדוח","מה יש בדוח","מה מוצג בדוח"],answer:"הדוח כולל: תאריכים - סוג הדיווח חופשה או WFH או מחלה - יתרה מצטברת ומכסה שנתית.",tags:"דוח,תוכן,אישי"},
  {question:"האם אפשר לייצא לאקסל?",aliases:["ייצוא excel","להוריד אקסל","קובץ אקסל"],answer:"כן - לשונית דוח אישי ואז ייצא לאקסל. ניתן גם לייצא מלוח המנהל לאדמינים.",tags:"אקסל,ייצוא,דוח"},
  {question:"מי יכול להוציא דוחות על כל העובדים?",aliases:["דוח כלל עובדים","מי מורשה","ייצוא מלא"],answer:"רק אדמין וחשב מורשים להוציא דוחות על כלל העובדים. מנהל מחלקה יכול לייצא דוח על עובדי מחלקתו.",tags:"הרשאות,דוח,אדמין"},
  {question:"איך מוסיפים עובד חדש?",aliases:["הוספת עובד","עובד חדש","רישום עובד"],answer:"לשונית ניהול - עובדים - לחץ הוסף עובד - מלא פרטים - שמור. העובד יקבל מייל הזמנה.",tags:"עובד חדש,הוספה,ניהול"},
  {question:"איך עורכים פרטי עובד?",aliases:["עריכת עובד","לשנות פרטים","עדכון עובד"],answer:"לשונית ניהול - חפש עובד - לחץ על שמו - ערוך - שנה את הפרטים הרצויים - שמור.",tags:"עריכה,עובד,ניהול"},
  {question:"איך מוחקים או משביתים עובד?",aliases:["מחיקת עובד","להשבית עובד","עובד שעזב"],answer:"לשונית ניהול - מצא עובד - השבת לשמירת היסטוריה או מחק לצמיתות. מומלץ השבת.",tags:"מחיקה,השבתה,עובד"},
  {question:"איך מגדירים מנהל מחלקה?",aliases:["מינוי מנהל","מנהל מחלקה","הגדרת מנהל"],answer:"לשונית ניהול - ניהול מחלקות ומנהלים - מצא המחלקה - בחר עובד מהרשימה כמנהל.",tags:"מנהל מחלקה,הגדרה,ניהול"},
  {question:"איך טוענים מכסות שנתיות?",aliases:["טעינת מכסות","הגדרת מכסה","אקסל מכסות"],answer:"לשונית ניהול - טעינת מכסות שנתיות - הורד תבנית אקסל - מלא username מכסה יתרה פתיחה - העלה.",tags:"מכסות,אקסל,טעינה"},
  {question:"מה הפורמט הנכון לקובץ מכסות?",aliases:["פורמט מכסות","עמודות בקובץ","מה לכתוב באקסל"],answer:"הקובץ צריך לכלול עמודות: username ו-annual_quota ו-initial_balance ו-balance_date. הורד את התבנית ממסך טעינת המכסות.",tags:"מכסות,פורמט,אקסל"},
  {question:"איך מאפסים סיסמת עובד?",aliases:["איפוס סיסמה","לאפס סיסמה","שכח סיסמה"],answer:"לשונית ניהול - מצא עובד - לחץ על שלוש הנקודות - אפס סיסמה.",tags:"סיסמה,איפוס,אבטחה"},
  {question:"איך משנים שם חברה?",aliases:["שם חברה","לשנות שם","עדכון חברה"],answer:"לשונית ניהול - הגדרות - שנה את שם החברה - שמור. השינוי ישתקף בכל הממשק.",tags:"הגדרות,שם חברה,מותאם אישית"},
  {question:"איך מעלים לוגו חברה?",aliases:["לוגו","להעלות לוגו","לוגו חברה"],answer:"לשונית ניהול - הגדרות - לוגו - לחץ על תיבת הלוגו - בחר קובץ תמונה - שמור.",tags:"לוגו,מותאם אישית,הגדרות"},
  {question:"איך עוברים למצב כהה?",aliases:["מצב לילה","dark mode","תצוגה כהה"],answer:"לחץ על אייקון הירח בפינה העליונה. ניתן גם להגדיר ברמת החברה בניהול - הגדרות.",tags:"dark mode,תצוגה,הגדרות"},
  {question:"האם המערכת עובדת בנייד?",aliases:["אפליקציה","נייד","מובייל","PWA"],answer:"כן - המערכת מותאמת לנייד ותומכת בהתקנה כ-PWA על מסך הבית.",tags:"נייד,PWA,נגישות"},
  {question:"האם הנתונים מסונכרנים בין מכשירים?",aliases:["סנכרון","בין מחשבים","כמה מכשירים"],answer:"כן - המערכת מסונכרנת ב-Firebase בזמן אמת. שינוי ממכשיר אחד מתעדכן מיידית בכל שאר המכשירים.",tags:"סנכרון,Firebase,זמן אמת"},
  {question:"מה קורה אם אין חיבור לאינטרנט?",aliases:["אופליין","ללא אינטרנט","מה קורה בלי רשת"],answer:"המערכת תמשיך לעבוד עם הנתונים האחרונים שנשמרו מקומית. בחזרת הקישוריות הנתונים יסתנכרנו אוטומטית.",tags:"אופליין,גיבוי,סנכרון"},
  {question:"איך מגבים את הנתונים?",aliases:["גיבוי נתונים","לשמור גיבוי","ייצוא נתונים מלא"],answer:"לשונית ניהול - גיבוי - ייצא גיבוי. יורד קובץ JSON עם כל נתוני המערכת.",tags:"גיבוי,ייצוא,אדמין"},
  {question:"איך משחזרים מגיבוי?",aliases:["שחזור גיבוי","לטעון גיבוי","לשחזר נתונים"],answer:"לשונית ניהול - גיבוי - שחזר מגיבוי - בחר קובץ JSON. שחזור יחליף את כל הנתונים הנוכחיים.",tags:"שחזור,גיבוי,ניהול"},
  {question:"מי יכול לגבות ולשחזר נתונים?",aliases:["הרשאת גיבוי","מי מורשה לגבות"],answer:"רק אדמין מורשה לבצע גיבוי ושחזור נתונים.",tags:"גיבוי,הרשאות,אדמין"},
  {question:"איך משנים סיסמה?",aliases:["שינוי סיסמה","סיסמה חדשה","להחליף סיסמה"],answer:"פרופיל אישי - לחץ על שמך - שנה סיסמה - הכנס סיסמה נוכחית ואחר כך חדשה.",tags:"סיסמה,אבטחה,פרופיל"},
  {question:"מה הדרישות לסיסמה חזקה?",aliases:["דרישות סיסמה","כמה תווים","עוצמת סיסמה"],answer:"מינימום 6 תווים. מומלץ לשלב אותיות מספרים ותווים מיוחדים.",tags:"סיסמה,אבטחה,דרישות"},
  {question:"מה ההבדל בין אדמין למנהל מחלקה?",aliases:["תפקידים","הרשאות תפקיד","מנהל מול אדמין"],answer:"אדמין - גישה מלאה לכל הנתונים ולהגדרות המערכת. מנהל מחלקה - רואה ומנהל רק את עובדי מחלקתו ומאשר בקשות.",tags:"הרשאות,תפקידים,אדמין מנהל"},
  {question:"מה עובד רגיל יכול לראות?",aliases:["הרשאות עובד","מה מותר לעובד","גישת עובד"],answer:"עובד רגיל רואה: יתרה ונתונים אישיים שלו - נעדרים מהחברה שמות וסטטוס יום - לוח שנה אישי - דוח אישי.",tags:"הרשאות,עובד,גישה"},
  {question:"מה מנהל מחלקה יכול לראות?",aliases:["הרשאות מנהל","מה רואה מנהל","גישת מנהל"],answer:"מנהל מחלקה רואה: כל נתוני עובדי מחלקתו כולל יתרות ונוכחות - בקשות ממתינות - דוחות מחלקתיים - תחזיות עומס.",tags:"הרשאות,מנהל,גישה"},
  {question:"האם ניתן לתת הרשאות מיוחדות לעובד?",aliases:["הרשאות מיוחדות","גישה מיוחדת","הרחבת הרשאות"],answer:"כן - לשונית ניהול - הרשאות - בחר עובד - הגדר הרשאות ספציפיות.",tags:"הרשאות,מיוחדות,ניהול"},
  {question:"מי רואה את לוג הפעולות?",aliases:["לוג","היסטוריית פעולות","מי רואה לוג"],answer:"רק אדמין יכול לראות את לוג הפעולות המלא.",tags:"לוג,הרשאות,אדמין"},
  {question:"מה זה פרוטוקול העברת מקל?",aliases:["handover","העברת מקל","פרוטוקול חופשה"],answer:"לפני יציאה לחופשה ניתן למלא פרוטוקול עם רשימת משימות ונקודות קריטיות ומחליף. המנהל רואה זאת בלוח המנהל.",tags:"handover,פרוטוקול,חופשה"},
  {question:"איך ממלאים פרוטוקול לפני חופשה?",aliases:["איך ממלאים handover","טופס לפני חופשה","להכין העברה"],answer:"לאחר אישור החופשה תופיע הודעה למילוי פרוטוקול. לחץ מלא פרוטוקול - הזן משימות ומחליף - שמור.",tags:"handover,מילוי,תהליך"},
  {question:"האם חובה למלא פרוטוקול?",aliases:["חובת handover","האם חייב","אפשר לדלג"],answer:"לא חובה טכנית אבל מומלץ מאוד. המנהל יכול לדרוש זאת.",tags:"handover,חובה,מדיניות"},
  {question:"מה זה Dazura?",aliases:["מה המערכת","מה זה המערכת","על מה המערכת"],answer:"Dazura היא מערכת ניהול חופשות ונוכחות לארגונים. מאפשרת לעובדים לנהל חופשות ולדווח נוכחות ולמנהלים לאשר ולנתח נתונים.",tags:"כללי,מערכת,Dazura"},
  {question:"האם המערכת שומרת מידע בענן?",aliases:["אחסון ענן","איפה הנתונים","אבטחת מידע"],answer:"כן - הנתונים מאוחסנים ב-Firebase של Google עם הצפנה מלאה. ניתן גם לעבוד מקומית ללא Firebase.",tags:"ענן,אבטחה,Firebase"},
  {question:"האם המערכת תומכת בעברית?",aliases:["שפת ממשק","עברית","תמיכה בעברית"],answer:"כן - המערכת כתובה בעברית מלאה עם תמיכה ב-RTL.",tags:"עברית,RTL,שפה"},
  {question:"למי פונים בבעיה טכנית?",aliases:["תמיכה טכנית","פנייה לתמיכה","עזרה טכנית"],answer:"פנה לאדמין המערכת בארגונך. לתקלות חמורות ניתן לפנות לצוות התמיכה של Dazura.",tags:"תמיכה,בעיה,פנייה"},
  {question:"מה ה-AI יכול לעשות?",aliases:["יכולות AI","מה אפשר לשאול AI","שאלות ל-AI"],answer:"ה-AI יכול: לחשב יתרות - להציג נעדרים - לנתח צוות - לחזות עומסים - לענות על שאלות על המערכת. הכל מהנתונים האמיתיים שלך.",tags:"AI,יכולות,מה אפשר"},
  {question:"האם ה-AI שומר את השיחות שלי?",aliases:["פרטיות AI","שמירת שיחות","מאגר שיחות"],answer:"ה-AI שומר את הקשר השיחה הנוכחית בלבד עד 20 הודעות אחורה. לאחר סגירת הפנל ההיסטוריה נמחקת.",tags:"AI,פרטיות,שיחות"},
  {question:"האם ה-AI מתחבר לאינטרנט?",aliases:["AI ורשת","האם AI גולש","חיבור חיצוני AI"],answer:"לא - ה-AI פועל מקומית לחלוטין על נתוני המערכת שלך. אין גישה לאינטרנט ואין שיתוף מידע עם שרתים חיצוניים.",tags:"AI,מקומי,פרטיות"},
  {question:"מי פיתח את ה-AI?",aliases:["מי בנה AI","מי יצר","מי אחראי AI"],answer:"ה-AI פותח על ידי מוטי קריחלי ומותאם לצרכי ניהול חופשות בארגונים ישראלים.",tags:"AI,פיתוח,יוצר"},
  {question:"מה המדיניות לגבי חופשה בתקופת שיא?",aliases:["חופשה בשיא","עונת שיא","תקופה עמוסה"],answer:"בתקופות שיא ייתכן שהמנהל יגביל חופשות. מומלץ לתאם מוקדם ולבדוק עם המנהל לפני הגשת הבקשה.",tags:"מדיניות,שיא,תכנון"},
  {question:"האם ניתן להעביר ימי חופשה לעמית?",aliases:["העברת ימים","לתת חופשה","שיתוף ימים"],answer:"לא - ימי חופשה אינם ניתנים להעברה בין עובדים. כל עובד מנהל את יתרתו האישית.",tags:"העברה,מדיניות,ימים"},
  {question:"האם יש הבדל בין עובד חדש לוותיק?",aliases:["עובד חדש מכסה","ותק וחופשה","שנה ראשונה"],answer:"המכסה השנתית עשויה להשתנות לפי ותק. הגדרת המכסה האישית נעשית על ידי אדמין.",tags:"ותק,מכסה,עובד חדש"},
  {question:"מתי מתאפסת יתרת החופשה?",aliases:["איפוס יתרה","תחילת שנה","מתי מתחילים מחדש"],answer:"בדרך כלל ב-1 בינואר אך תלוי בהגדרות המנהל. חלק מהחברות מעבירות יתרה משנה לשנה.",tags:"איפוס,יתרה,שנה חדשה"},
  {question:"האם ימי חופשה נרכשים ברמה חודשית?",aliases:["צבירה חודשית","כמה בחודש","מתי נצברים ימים"],answer:"כן - המכסה השנתית מחולקת ל-12 ונצברת מדי חודש.",tags:"צבירה,חודשי,חישוב"},
  {question:"מה קורה לימי חופשה שלא נוצלו בסוף שנה?",aliases:["ימים לסוף שנה","יתרה בסוף שנה","מה עם הימים שנשארו"],answer:"תלוי במדיניות: חלק מהחברות מאפשרות העברה לשנה הבאה - אחרות מפצות כספית - ויש שמבטלות. בדוק עם ההנהלה.",tags:"סוף שנה,יתרה,מדיניות"},
  {question:"מה יש בלשונית סקירה?",aliases:["לשונית סקירה","דשבורד עובד","מה יש בסקירה"],answer:"לשונית סקירה מציגה: יתרה נוכחית - ניצול השנה - חופשות קרובות - סטטוס הבקשה האחרונה ותחזית סוף שנה.",tags:"סקירה,דשבורד,ניווט"},
  {question:"מה יש בלשונית לוח מנהל?",aliases:["לוח מנהל","לשונית מנהל","דשבורד מנהל"],answer:"לוח המנהל כולל: נוכחות צוות היום - בקשות ממתינות לאישור - יתרות כל העובדים - תחזיות עומסים וציוני רווחה.",tags:"מנהל,לוח,ניווט"},
  {question:"מה יש בלשונית ניהול?",aliases:["לשונית ניהול","אדמין","מה יש בניהול"],answer:"לשונית ניהול כוללת: ניהול עובדים - מחלקות - מכסות - הרשאות - הגדרות מערכת - גיבוי ו-Firebase.",tags:"ניהול,אדמין,ניווט"},
  {question:"מה יש בלשונית שעון נוכחות?",aliases:["שעון נוכחות","לשונית שעון","מה עושים בשעון"],answer:"לשונית שעון נוכחות מאפשרת: כניסה ויציאה - דיווח WFH - דיווח מחלה - צפייה בהיסטוריית נוכחות אישית.",tags:"שעון,נוכחות,ניווט"},
  {question:"מה יש בלשונית דוח אישי?",aliases:["דוח אישי","לשונית דוח","מה בדוח"],answer:"דוח אישי מציג: היסטוריית כל הדיווחים שלך - פירוט חודשי - ניתוח ניצול לפי סוג ואפשרות ייצוא.",tags:"דוח אישי,ניווט,ייצוא"},
  {question:"מי בחופשה היום?",aliases:["מי נעדר היום","מי לא מגיע","מי יצא לחופש"],answer:"שאל את ה-AI: מי בחופשה היום - תקבל רשימה מסודרת לפי סוג היעדרות.",tags:"נוכחות,היום,נעדרים"},
  {question:"מי עובד מהבית היום?",aliases:["מי WFH היום","מי מהבית","מי לא פיזי"],answer:"שאל את ה-AI: מי עובד מהבית היום - יוצגו כל העובדים שדיווחו WFH.",tags:"WFH,היום,נוכחות"},
  {question:"כמה עובדים יש בחברה?",aliases:["מספר עובדים","מצבת","סהכ עובדים"],answer:"שאל את ה-AI: כמה עובדים בחברה - יוצג מספר עובדים פעילים עם פירוט נוכחות היום.",tags:"עובדים,מספר,חברה"},
  {question:"מה מצב הצוות השבוע?",aliases:["מצב שבוע","מי בחופשה השבוע","מי נעדר השבוע"],answer:"שאל את ה-AI: מה מצב הצוות השבוע - יציג נעדרים לכל ימי השבוע.",tags:"מצב,שבוע,צוות"},
  {question:"אילו מחלקות יש בחברה?",aliases:["רשימת מחלקות","מחלקות החברה","כמה מחלקות"],answer:"שאל את ה-AI: אילו מחלקות יש בחברה - יוצגו כל המחלקות הרשומות.",tags:"מחלקות,רשימה,חברה"},
  {question:"מי במחלקה שלי?",aliases:["חברי הצוות","מי בצוות","אנשי המחלקה"],answer:"שאל את ה-AI: מי במחלקה שלי - יוצגו כל חברי המחלקה עם סטטוס יום.",tags:"מחלקה,צוות,עמיתים"},
  {question:"מה תחזית העומסים ל-8 שבועות?",aliases:["תחזית עומס","8 שבועות","חיזוי נוכחות"],answer:"שאל את ה-AI: תחזית עומסים ל-8 שבועות - יציג כמה עובדים צפויים להיעדר בכל שבוע.",tags:"תחזית,עומס,מנהל"},
  {question:"אילו עובדים בסיכון שחיקה?",aliases:["שחיקה","burnout","לא לקח חופשה"],answer:"שאל את ה-AI: מי בסיכון שחיקה - מציג עובדים שלא לקחו חופשה מעל 90 יום.",tags:"שחיקה,רווחה,מנהל"},
  {question:"כמה בקשות ממתינות לאישור?",aliases:["ממתינות","בקשות פתוחות","כמה צריך לאשר"],answer:"שאל את ה-AI: כמה בקשות ממתינות - יציג מספר ושמות המבקשים.",tags:"בקשות,ממתינות,מנהל"},
  {question:"מה עלות החופשות הצבורות בחברה?",aliases:["עלות חופשות","חבות כספית","עלויות"],answer:"שאל את ה-AI: מה עלות החופשות הצבורות - יחשב חבות כספית לפי יתרות ושכר יומי.",tags:"עלות,חבות,כספי"},
  {question:"מה המחלקה עם הנעדרים הגבוהים היום?",aliases:["מחלקה עמוסה","עומס מחלקה","הכי הרבה נעדרים"],answer:"שאל את ה-AI: איזו מחלקה הכי עמוסה היום - מציג את 3 המחלקות עם אחוז ההיעדרות הגבוה ביותר.",tags:"מחלקה,עומס,מנהל"},
  {question:"מי לא לקח חופשה השנה?",aliases:["לא ניצל חופשה","אפס ניצול","לא יצא לחופש"],answer:"שאל את ה-AI: מי לא לקח חופשה השנה - מציג עובדים עם ניצול אפס.",tags:"ניצול,חופשה,מנהל"},
  {question:"כיצד שולחים הודעה לכל העובדים?",aliases:["הודעה לכולם","שליחת הודעה","הודעה כללית"],answer:"לשונית ניהול - הודעות - שלח הודעה חדשה. ניתן לשלוח לכלל העובדים או למחלקה ספציפית.",tags:"הודעות,תקשורת,מנהל"},
  {question:"האם עובדים מקבלים התראות?",aliases:["התראות","notifications","עדכונים"],answer:"כן - המערכת מציגה התראות פנימיות: אישור או דחיית בקשה - הודעות מנהל - תזכורות לפרוטוקול.",tags:"התראות,עדכונים,מערכת"},
  {question:"כיצד מוסיפים חגים ללוח שנה?",aliases:["הוספת חגים","חגים לוח","ימי חג"],answer:"לשונית ניהול - הגדרות - חגים. ניתן להוסיף ולנהל ימי חג בהתאם ללוח השנה הישראלי.",tags:"חגים,לוח שנה,ניהול"},
  {question:"מה קורה אם מחקתי נתונים בטעות?",aliases:["מחיקה בטעות","שחזור נתונים","undo"],answer:"אם יש גיבוי עדכני ניתן לשחזר דרך ניהול - גיבוי - שחזר מגיבוי. לכן חשוב לגבות בקביעות.",tags:"מחיקה,שחזור,גיבוי"},
  {question:"האם חופשת מחלה ארוכה משפיעה על הצבירה?",aliases:["מחלה ארוכה","חופשת מחלה","מחלה וצבירה"],answer:"ימי מחלה אינם מפסיקים את צבירת ימי החופשה - הצבירה ממשיכה בזמן מחלה.",tags:"מחלה,צבירה,חישוב"},
  {question:"איך מחברים Firebase?",aliases:["הגדרת Firebase","חיבור ענן","Firebase config"],answer:"לשונית ניהול - Firebase - הכנס את פרטי הפרויקט apiKey ו-projectId וכו.",tags:"Firebase,חיבור,הגדרות"},
  {question:"האם יש תקרה לצבירת ימי חופשה?",aliases:["תקרת צבירה","מקסימום ימים","כמה אפשר לצבור"],answer:"תלוי במדיניות החברה. אדמין יכול להגדיר תקרת צבירה. פנה למשאבי אנוש לבדיקה.",tags:"תקרה,צבירה,מדיניות"},
  {question:"האם אפשר לפצל חופשה לכמה תקופות?",aliases:["פיצול חופשה","חופשה בחלקים","מספר תקופות"],answer:"כן - ניתן להגיש בקשות נפרדות לתקופות שונות. כל בקשה תאושר בנפרד.",tags:"פיצול,חופשה,תכנון"},
];
...
`**📡 שאלות שאני עונה עליהן:**\n• "מי מהצוות בחופשה / WFH היום? מחר? ביום שלישי?"\n• "מה סטטוס הבקשה שלי?"\n• "מה הימים המומלצים לחופש?"\n• "מה שם החברה?" / "גרסת המערכת"\n• "איך משנים סיסמה?" / "איך מעדכנים מייל?"\n\n💡 כתוב בחופשיות — אני מבין עברית טבעית`;
  ...
  function respond(rawInput, currentUser, db) {
    ...
    // ── Custom Q&A + Builtin Q&A — בדיקה ראשונה לפני כל intent ──
    // מיזוג: db.customQA (מהמשתמש) + BUILTIN_QA (מובנה) — customQA גובר
    const _allQA = [
      ...(db && db.customQA ? db.customQA : []),
      ...(typeof BUILTIN_QA !== 'undefined' ? BUILTIN_QA : [])
    ];
    if (_allQA.length) {
      const _t = (rawInput || '').toLowerCase().replace(/[֑-ׇ]/g, '').trim();
      for (const entry of _allQA) {
        const _q = (entry.question || '').toLowerCase().replace(/[֑-ׇ]/g, '').trim();
        const _matched = _t === _q ||
          (_t.includes(_q) && _q.length >= 4) ||
          (_q.includes(_t) && _t.length >= 4) ||
          (entry.aliases || []).some(a => {
            const _an = (a || '').toLowerCase().replace(/[֑-ׇ]/g, '').trim();
            return _an.length >= 3 && (_t === _an || _t.includes(_an) || _an.includes(_t));
          });
        if (_matched) {
          const _r = entry.answer;
          conversationHistory.push({ role: 'ai', text: _r });
          return _r;
        }
      }
    }
