...
const DazuraAI = (() => {
  ...
  const INTENT_RULES = [
    ...
    // WHO + any date
    { name:'who_vacation',  score: t=>/מי (ב|הוא|היא|נמצא|יצא|בחופשה|חופש|נופש|נהנה|נעלם|נח|נופשים?|נהנים?|נחים?)|מי חופשה|מי יצא (לחופש|לנפוש|לנוח|ליהנות|מוקדם)|מי לא (מגיע|הגיע|כאן|נמצא|עובד|התייצב|נראה)|מי נעדר|מי חסר|מי לא פה|מי אין|אין היום|נעדרים היום|מי נהנה|מי נופש/.test(t)?10:0 },
    { name:'who_wfh',       score: t=>/מי (עובד מהבית|ב.?wfh|מהבית|remote)|wfh|מי מהבית/.test(t)?10:0 },
    { name:'who_sick',      score: t=>/מי חולה|מי (ב)?מחלה|מי מחלה|מי לא בסדר|מי סובל|מי (ב)?בית חולים|מי ביטל.*היום/.test(t)?10:0 },
    { name:'who_office',    score: t=>/מי במשרד|מי (בחברה|בעבודה|בפנים|נוכח|נמצא.*עבודה|הגיע.*היום)|נוכחות|מי (פיזי|מגיע|כאן|פה|התייצב)|כמה (במשרד|בחברה|נוכחים)/.test(t)?10:0 },
    { name:'team_status',   score: t=>/מצב (הצוות|הארגון|החברה|כולם|הכל)|הצוות (היום|מחר|השבוע)|עמיתי|חברי הצוות|מה קורה.*היום|מה המצב.*היום|מה נשמע.*עבודה|מי (כאן|פה|נמצא).*היום|מצב כללי|דשבורד|סקירה כללית|מה יש היום|מה קורה פה/.test(t)?10:0 },
    // Admin
    ...
    { name:'greeting',      score: t=>/^(שלום|היי|הי|הלו|בוקר|ערב|צהריים|לילה|מה נשמע|מה מצבך|מה קורה|מה המצב|מה שלומך|יש חיים|ניתן לשאול|יש מישהו|מה יש פה)\s*/.test(t)?10:0 },
  ...
  function detectIntent(text) {
    // נסה קודם DazuraKnowledge (ai-knowledge.js)
    if (typeof DazuraKnowledge !== 'undefined') {
      const result = DazuraKnowledge.detectIntent(text);
      if (result.intent !== 'unknown' && result.score >= 8) return result.intent;
    }
    // fallback ל-INTENT_RULES המקוריים
...
דוגמה: שינוי מ-20 ל-24 ימים ב-1.7 → מוסיף ~2 ימים (חצי שנה).`,
  ...
  function respondThanks(user) {
    const n = user.fullName.split(' ')[0];
    if (typeof DazuraKnowledge !== 'undefined') {
      const pool = DazuraKnowledge.THANKS_RESPONSES;
      return pool[Math.floor(Math.random() * pool.length)](n);
    }
    const phrases = [
      `בשמחה, ${n}! 😊 אם יש עוד שאלה — אני כאן.`,
      `תמיד! אם תרצה לדעת עוד — רק תשאל.`,
  ...
  function respondConfused(user, rawInput) {
    const n = user.fullName.split(' ')[0];
    if (typeof DazuraKnowledge !== 'undefined') {
      const pool = DazuraKnowledge.CONFUSED_RESPONSES;
      const hint = DazuraKnowledge.guessIntent(rawInput || '');
      const resp = pool[Math.floor(Math.random() * pool.length)](n, hint);
      return resp;
    }
...
אם תרצה רשימת דברים שאני יכול לענות — כתוב **"מה אתה יכול?"**`;
  ...
  function respondWhoIsMoti(user) {
    ...
    function respondGreeting(user, rawInput) {
    const n = user.fullName.split(' ')[0];
    if (typeof DazuraKnowledge !== 'undefined') {
      const pool = DazuraKnowledge.GREETING_RESPONSES;
      const tc = DazuraKnowledge.getTimeContext(rawInput);
      const resp = pool[Math.floor(Math.random() * pool.length)](n, tc);
      return resp + `\n\nאני יכול לענות על שאלות חופשות, נוכחות ועוד. שאל/י בחופשיות.`;
    }
    const h = new Date().getHours();
...
`**📡 שאלות שאני עונה עליהן:**\n• "מי מהצוות בחופשה / WFH היום? מחר? ביום שלישי?"\n• "מה סטטוס הבקשה שלי?"\n• "מה הימים המומלצים לחופש?"\n• "מה שם החברה?" / "גרסת המערכת"\n• "איך משנים סיסמה?" / "איך מעדכנים מייל?"\n\n💡 כתוב בחופשיות — אני מבין עברית טבעית`;
  ...
  function respondUnknown(rawInput, currentUser, db) {
    ...
    // ── 6. Default — תשובה חכמה לפי תפקיד ─────────────────
    // במקום הצעה יבשה — תן מידע אמיתי
    const today2 = dateToKey(new Date());
    const allStats2 = getStatsForDate(db, today2);

    if (!isAdmin && !isManager) {
      // עובד — הצג מצב היום + שאל מה רצה
      const absent = [...allStats2.vacation, ...allStats2.sick, ...allStats2.wfh];
      const absentStr = absent.length
        ? `היום נעדרים **${absent.length}** עובדים.`
        : 'כולם במשרד היום 📍';
      return `${firstName}, לא הבנתי לגמרי 🙏 — אבל הנה מה שאני יודע:
${absentStr}

נסה לשאול בצורה ספציפית יותר, למשל:
• "מי בחופשה היום?"
• "מה היתרה שלי?"
• "מה אתה יכול?" — לרשימה מלאה`;
    }

    if (isManager) {
      const pending2 = (db.approvalRequests || []).filter(r => r.status === 'pending').length;
      return `${firstName}, לא הבנתי בדיוק מה ביקשת 🙏

מה שאני יודע עכשיו:
📍 במשרד: ${allStats2.office.length} | 🏖️ חופשה: ${allStats2.vacation.length} | 🏠 WFH: ${allStats2.wfh.length} | 🤒 מחלה: ${allStats2.sick.length}
⏳ ממתינות לאישור: **${pending2}**

נסה: "מי בחופשה מחר?" / "בקשות ממתינות" / "מה אתה יכול?"`;
    }

    // Admin
    return `${firstName}, לא הבנתי את השאלה 🙏
נסה: "מי בחופשה היום?" / "ציוני רווחת עובדים" / "מה אתה יכול?"`;
  }
  ...
  function detectFollowUp(text) {
    if (typeof DazuraKnowledge !== 'undefined') {
      const fu = DazuraKnowledge.detectFollowUp(text);
      if (fu) return fu;
    }
    // fallback legacy
    const t = text.trim();
  ...
  function handleFollowUp(followUpType, currentUser, db) {
    ...
    // המשך עם תאריך מחר
    if (followUpType === 'tomorrow_same' && ctx.intent) {
      const tomorrow = new Date(); tomorrow.setDate(tomorrow.getDate()+1);
      const tomorrowInfo = { date: tomorrow, label: 'מחר', single: true };
      if (ctx.intent.includes('vacation') || ctx.intent === 'team_status') {
        return respondWhoAt(db, tomorrowInfo, currentUser, null);
      }
    }

    // המשך עם שבוע הבא
    if (followUpType === 'next_week_same' && ctx.intent) {
      const start = new Date(); start.setDate(start.getDate()+(7-start.getDay()+1)%7+1);
      const end = new Date(start); end.setDate(start.getDate()+6);
      const weekInfo = { dateStart: start, dateEnd: end, label: 'שבוע הבא', single: false, range: true };
      return respondWhoAtRange(db, weekInfo, currentUser, null);
    }

    // כמה ימים יש לו/לה
    if (followUpType === 'his_balance' && ctx.subject) {
      const u = db.users[ctx.subject];
      if (u) return respondEmpBalance(u, db, new Date().getFullYear());
    }

    // הפניה לעובד ("הוא"/"היא")
    if (followUpType === 'ref_he' && ctx.subject) {
      const u = db.users[ctx.subject];
      if (u) return respondEmpBalance(u, db, new Date().getFullYear());
    }
  ...
  function respond(rawInput, currentUser, db) {
    ...
    let response='';
    switch(intent) {
      case 'greeting':        response=respondGreeting(currentUser, rawInput); break;
      case 'who_is_moti':     response=respondWhoIsMoti(currentUser); break;
      case 'help':            response=respondHelp(currentUser); break;
      case 'who_am_i':
        response=respondWhoAmI(currentUser,db);
        lastContext={...lastContext, intent, resultList:[], subject:currentUser.username, subjectName:currentUser.fullName, lastQuestion:rawInput, turnCount:(lastContext.turnCount||0)+1};
        break;
      case 'my_dept': {
      ...
      case 'emp_balance': {
        ...
        lastContext={...lastContext, intent, subject:uname, subjectName:db.users[uname]?.fullName, lastQuestion:rawInput};
      ...
      case 'confused':        response=respondConfused(currentUser, rawInput); break;
      case 'faq_company_name':
      case 'faq_version':
      case 'faq_send_message':
      case 'faq_time_who':
...
• אשר בסבבים ולא הכל בבת אחת`; break;
  ...
  function clearHistory() {
    conversationHistory=[];
    lastContext = typeof DazuraKnowledge !== 'undefined'
      ? DazuraKnowledge.EMPTY_CONTEXT()
      : {intent:null,dateInfo:null,resultList:[],subject:null,subjectName:null,dept:null,data:null,lastQuestion:null,turnCount:0};
  }
