// ============================================================
// DAZURA AI ENGINE v5.1 + FUSE
// Built by מוטי קריחלי 🏆
//
// ארכיטקטורה — Pipeline של 5 שלבים:
...
const DazuraAI = (() => {
  ...
  function respondHelp(user, db) {
    ...
    if (mgr||adm) out += `🏢 **מנהל** — "תחזית עומסים" · "מי בסיכון שחיקה?" · "בקשות ממתינות" · יתרה לפי שם עובד\n`;
    if (adm)      out += `⚙️ **ניהול** — "איך מוסיפים עובד?" · "איך מחברים Firebase?" · "איך מגבים?"\n`;
...
` +
  ...
  function runKnowledge(raw) {
    ...
    if (typeof AI_KNOWLEDGE !== 'undefined' && AI_KNOWLEDGE.length) {
      for (const entry of AI_KNOWLEDGE) {
        ...
        // הוראת DB — החזר null כדי ש-runLiveData יטפל
  ...
  function clearHistory() {
    ...
    _kbFuse = null; // איפוס cache של Fuse index
  }
